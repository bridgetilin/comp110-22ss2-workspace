...
#     i: int = 0 
#     matches: int = 0  # variable to keep track of the number of elements that are equal at the same index between the the two input lists
#     if len(list_1) != len(list_2):  
#         return False
#     while i < len(list_1):  # check each index
#         if list_1[i] == list_2[i]:
#             matches += 1  # each time there is a match at the same index, add one to the number of matches
#         i += 1 
#     if matches == len(list_1):  # if the number of matches is the same as the length of the list, each index matches and the 2 lists are deeply equal 
#         return True  
#     else:
#         return False 
...
#     """Given a list of integers and a start and end index, return the subset list of the given list that is between the specified start index and one less than the end index."""
